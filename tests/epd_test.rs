...
const EPD_POSITIONS: &[(&str, &str)] = &[
    ...
    ("rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 1", "exd5 e5 Nc3 Nf3"),
    
    // Posizione 3: Sviluppo pezzi
    ("r1bqkbnr/pppp1ppp/2n5/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 0 1", "O-O d3 Nc3 h3"),
...
fn is_reasonable_move(fen: &str, bestmove: &str) -> bool {
    ...
    let from_file = bestmove.chars().nth(0).unwrap_or(' ');
    ...
    // File: a-h, Rank: 1-8
    let valid_from_file = from_file >= 'a' && from_file <= 'h';
    let valid_from_rank = from_rank >= '1' && from_rank <= '8';
    let valid_to_file = to_file >= 'a' && to_file <= 'h';
    let valid_to_rank = to_rank >= '1' && to_rank <= '8';
        ...
        match search_position(engine, fen, depth) {
            Some(result) => {
                if is_reasonable_move(fen, &result.bestmove) {
                    println!("✅ PASS (found: {}, score: {})", result.bestmove, result.score);
                    passed += 1;
                } else {
...
fn test_material_evaluation() {
    ...
    // Posizione pari
    let even_pos = search_position(engine, "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", depth);
    assert!(even_pos.is_some());
    let even_score = even_pos.unwrap().score;
    println!("Posizione pari: score={}", even_score);
    assert!(even_score.abs() < 100, "Posizione pari dovrebbe avere score ≈ 0, trovato {}", even_score);
    
    // Posizione con pedone in più per bianco
    let white_plus_pawn = search_position(engine, "rnbqkbnr/ppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", depth);
    assert!(white_plus_pawn.is_some());
    let white_score = white_plus_pawn.unwrap().score;
    println!("Bianco +pedone: score={}", white_score);
    assert!(white_score >= 50, "Bianco con pedone in più dovrebbe avere score positivo, trovato {}", white_score);
    
    // Posizione con pedone in più per nero
    let black_plus_pawn = search_position(engine, "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPP/RNBQKBNR w KQkq - 0 1", depth);
    assert!(black_plus_pawn.is_some());
    let black_score = black_plus_pawn.unwrap().score;
    println!("Nero +pedone: score={}", black_score);
    assert!(black_score < -50, "Nero con pedone in più dovrebbe avere score negativo per bianco, trovato {}", black_score);
}
...
fn test_search_consistency() {
    ...
    assert_eq!(res1.bestmove, res2.bestmove, 
        "Bestmove diverso tra run: {} vs {}", res1.bestmove, res2.bestmove);
    assert!((res1.score - res2.score).abs() < 5,
        "Score troppo diverso tra run: {} vs {}", res1.score, res2.score);
}
