...
fn main() {
    ...
    board
        .set_from_fen("rnbqkbnr/ppp2ppp/4p3/3pP3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 3")
        .unwrap();

    println!("1. WITH quiescence (depth=4):");
    let mut search_with_q = Search::new(
        board.clone(),
        1,
        SearchParams::new().max_depth(4).qsearch_depth(6),
    );
    ...
    let mut search_no_q = Search::new(
        board.clone(),
        1,
        SearchParams::new().max_depth(4).qsearch_depth(0),
    );
