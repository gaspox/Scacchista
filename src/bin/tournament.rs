...
fn main() {
    ...
    for i in 1..=rounds {
        ...
        print!("Game {}/{} ({} vs {})... ", i, rounds, white_name, black_name);
        std::io::stdout().flush().unwrap();

        match play_game(white_path, black_path, time_ms, inc_ms) {
            GameResult::WhiteWin => {
                println!("1-0 (White wins)");
                if white_name == "v0.5" { score1 += 1.0; } else { score2 += 1.0; }
            },
            GameResult::BlackWin => {
                println!("0-1 (Black wins)");
                if black_name == "v0.5" { score1 += 1.0; } else { score2 += 1.0; }
            },
            GameResult::Draw => {
...
fn play_game(white_path: &str, black_path: &str, time_ms: u64, inc_ms: u64) -> GameResult {
    ...
    let mut last_move_str = String::new();
    ...
    for _ in 0..200 {
        ...
        let go_cmd = format!("go wtime {} btime {} winc {} binc {}", wtime, btime, inc_ms, inc_ms);
        ...
        if turn == 0 { // White
            ...
            if wtime > real_elapsed { wtime -= real_elapsed; } else { wtime = 0; }
            wtime += inc_ms;
            ...
            if score.abs() >= 29000 {
                 ...
                 if score > 0 { return GameResult::WhiteWin; } else { return GameResult::BlackWin; }
            }
            moves.push(mv.clone());
            last_move_str = mv;
            turn = 1;
            ...
            let real_elapsed = start.elapsed().as_millis() as u64;
            if btime > real_elapsed { btime -= real_elapsed; } else { btime = 0; }
            btime += inc_ms;
            ...
            if score.abs() >= 29000 {
                 println!(" [Mate detected]");
                 if score > 0 { return GameResult::BlackWin; } else { return GameResult::WhiteWin; }
            }
            moves.push(mv.clone());
            last_move_str = mv;
            turn = 0;
